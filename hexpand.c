...
char* sha1_extend(EVP_MD_CTX *mdctx, char* signature, int length) {
	...
	int length_modulo = EVP_MD_block_size(md);
	...
	unsigned char* h_data = (unsigned char *)((SHA512_CTX *)EVP_MD_CTX_md_data(mdctx))->h;
	int h_data_size = EVP_MD_size(md);
	...
	if (length_modulo == 128) sprintf(output+2*padding, "%032lu", htole64(8*length));
	else sprintf(output+2*padding, "%016lu", htole64(8*length));
...
char* md5_extend(EVP_MD_CTX *mdctx, char* signature, int length) {
	...
	memset(data, 'A', length+padding+length_bytes);
	EVP_DigestUpdate(mdctx, data, length+padding+length_bytes);
	((MD5_CTX *)EVP_MD_CTX_md_data(mdctx))->A = htonl(strntoul(signature, 8, 16));
	((MD5_CTX *)EVP_MD_CTX_md_data(mdctx))->B = htonl(strntoul(signature+8, 8, 16));
	((MD5_CTX *)EVP_MD_CTX_md_data(mdctx))->C = htonl(strntoul(signature+16, 8, 16));
	((MD5_CTX *)EVP_MD_CTX_md_data(mdctx))->D = htonl(strntoul(signature+24, 8, 16));
